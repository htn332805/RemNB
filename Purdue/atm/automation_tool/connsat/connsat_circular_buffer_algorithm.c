...
// #define CIRCULAR_BUFFER_DUMP_DEBUG 1   /* remove // to enable debug */
...
// read file "aaaa", create-truncate and write output file "bbbb1", "bbbb2",
// "bbbb3"
...
// @@@
    ...
    if (input_eof == 0) {
      ...
      r = circular_buffer_stream_read(circular_buffer_01_p,
                                      fd_in, /* input_fd */
                                      &read_flags);
    ...
    r = circular_buffer_block_transfer(circular_buffer_01_p,
    ...
    r = circular_buffer_block_transfer(circular_buffer_02_p,
    ...
    printf("%4d: We Think BUFFER-03 can output up to %d bytes" NEWLINE,
           loop_count, circular_buffer_03_p->max_readable_chunk);

    printf("%4d: We Think BUFFER-14 can input up to %d bytes" NEWLINE,
           loop_count, circular_buffer_14_p->max_writeable_chunk);

    printf("%4d: We Think BUFFER-24 can input up to %d bytes" NEWLINE,
           loop_count, circular_buffer_24_p->max_writeable_chunk);
#endif
    ...
    printf("%4d: max_multi_buffer_transfer is up to %d bytes" NEWLINE,
           loop_count, max_multi_buffer_transfer);
    ...
    if (max_multi_buffer_transfer >= 1) {
...
#else
      r = circular_buffer_multi_block_transfer(circular_buffer_03_p,
                                               circular_buffer_04_p,
      ...
      r = circular_buffer_multi_block_transfer_complete(
          circular_buffer_03_p, max_multi_buffer_transfer);
    }
#else
    /* transfer buffer_03 to buffer_04 */
    r = circular_buffer_block_transfer(circular_buffer_03_p,
                                       circular_buffer_04_p);
#endif
    ...
    r = circular_buffer_stream_write(circular_buffer_04_p,
                                     fd_out_1 /* output_fd */
    );
    ...
    r = circular_buffer_stream_write(circular_buffer_14_p,
                                     fd_out_2 /* output_fd */
    );

    /* write output_fd from buffer_24 */
    r = circular_buffer_stream_write(circular_buffer_24_p,
                                     fd_out_3 /* output_fd */
    );
#endif
...
#ifdef CIRCULAR_BUFFER_THROUGPUT_TRACE
    printf("" NEWLINE);
    printf("TOTAL-BUFFERED_BYTES: %d, TOTAL-LOOP-COUNT: %d" NEWLINE,
           total_buffered_bytes, loop_count);
    printf("" NEWLINE);
#endif
  ...
  for (circular_buffer_used_bytes = 0;
       circular_buffer_used_bytes <= circular_buffer_allocated_size;
    ...
    total_free_space =
        circular_buffer_allocated_size - circular_buffer_used_bytes;

#ifdef CIRCULAR_BUFFER_ALGORITHM_DEBUG
    printf("total_used_space==%d, total_free_space==%d" NEWLINE,
           total_used_space, total_free_space);
    printf("" NEWLINE);
#endif

    /* for every possible start point in the allocated buffer... */
    for (unwrapped_read_start_ptr = 0;
         unwrapped_read_start_ptr < circular_buffer_allocated_size;
         ++unwrapped_read_start_ptr) {

      r = circular_buffer_math_calculations(
          circular_buffer_allocated_size, unwrapped_read_start_ptr,
          circular_buffer_used_bytes, &max_readable_chunk,
          &wrapped_write_start_ptr, &max_writeable_chunk);

#ifdef CIRCULAR_BUFFER_ALGORITHM_DEBUG
      printf("\t\tcircular_buffer_allocated_size==%d, "
             "unwrapped_read_start_ptr==%d, "
             "circular_buffer_used_bytes==%d," NEWLINE
             "\t\t&max_readable_chunk==%d, &wrapped_write_start_ptr==%d, "
             "&max_writeable_chunk==%d" NEWLINE,
...
int circular_buffer_init(char *new_buffer_name_p, int allocated_size,
  ...
  // @@@
  // printf("CREATED CIRCULAR BUFFER HEADER-%s, %ld bytes, address==%p"NEWLINE,
  // new_buffer_name_p, sizeof(circular_buffer_t), create_circular_buffer_p);
  ...
  // new_buffer_name_p, allocated_size,
  // create_circular_buffer_p->circular_buffer_data_p);

  memset(create_circular_buffer_p->circular_buffer_data_p, 0, allocated_size);

  strncpy(create_circular_buffer_p->buffer_name, new_buffer_name_p,
          sizeof(create_circular_buffer_p->buffer_name));
...
int circular_buffer_dump(circular_buffer_t *circular_buffer_p) {
  printf("\t\t%s==%d, unwrapped_read_start_ptr==%d, "
         "circular_buffer_used_bytes==%d," NEWLINE
         "\t\t&max_readable_chunk==%d, &wrapped_write_start_ptr==%d, "
         "&max_writeable_chunk==%d" NEWLINE,
...
int circular_buffer_view(circular_buffer_t *circular_buffer_p,
                         char **buffer_view_pp, int *buffer_view_size_p) {
  ...
  // BUG: memset(circular_buffer_p, 0,
  // circular_buffer_p->circular_buffer_used_bytes + 2);
  ...
  for (i = 0; i < circular_buffer_p->circular_buffer_used_bytes; ++i) {
    ...
    malloc_buffer_p[i] =
        circular_buffer_p->circular_buffer_data_p[buffer_index_offset];
  }
...
int circular_buffer_calculations(circular_buffer_t *circular_buffer_p) {
  int r;

  // @@@
  // printf("CALC: CIRCULAR BUFFER HEADER-%s, %ld bytes, address==%p"NEWLINE,
  // circular_buffer_p->buffer_name, sizeof(circular_buffer_t),
  // circular_buffer_p);
  ...
  CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(circular_buffer_p);
  if (circular_buffer_p->unwrapped_read_start_ptr >=
      circular_buffer_p->circular_buffer_allocated_size) {

#ifdef CIRCULAR_BUFFER_WRAP_DEBUG
    printf("ERROR_1: %s-%d: unwrapped_read_start_ptr==%d, "
           "circular_buffer_allocated_size==%d" NEWLINE,
           __FILE__, __LINE__, circular_buffer_p->unwrapped_read_start_ptr,
           circular_buffer_p->circular_buffer_allocated_size);
#endif
...
#ifdef CIRCULAR_BUFFER_WRAP_DEBUG
    printf("ERROR_1: %s-%d: unwrapped_read_start_ptr<0==%d, "
           "circular_buffer_allocated_size==%d" NEWLINE,
           __FILE__, __LINE__, circular_buffer_p->unwrapped_read_start_ptr,
           circular_buffer_p->circular_buffer_allocated_size);
#endif
...
int circular_buffer_stream_read(circular_buffer_t *circular_buffer_p, int fd,
                                int *flags_p) {
  int i;
  int r;
...
int circular_buffer_stream_write(circular_buffer_t *circular_buffer_p, int fd) {
  int i;
  int r;
  ...
  if (circular_buffer_p->calculated_buffer_max_readable_chunk >= 1) {
    ...
    if (r == -1) {
      ...
      for (i = 0; i < r; ++i) {
        ...
        // WRAP@@@@@@@
        CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(circular_buffer_p);
        putc(circular_buffer_p->circular_buffer_data_p
                 [(circular_buffer_p->unwrapped_read_start_ptr + i) %
                  circular_buffer_p->circular_buffer_allocated_size],
             stdout);
      }
      printf("\"" NEWLINE);
...
#ifdef CIRCULAR_BUFFER_WRAP_DEBUG
        printf("FIX_1: %s-%d: "
               "circular_buffer_p->unwrapped_read_start_ptr==%d" NEWLINE,
               __FILE__, __LINE__, circular_buffer_p->unwrapped_read_start_ptr);
#endif
      }
...
int circular_buffer_getc(circular_buffer_t *circular_buffer_p,
                         int *getc_char_p) {
  int r;
  int read_count;
  ...
  if (circular_buffer_p->calculated_buffer_max_readable_chunk >= 1) {
    ...
    CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(circular_buffer_p);
    // write fd into buffer */
    *getc_char_p = circular_buffer_p->circular_buffer_data_p
                       [circular_buffer_p->unwrapped_read_start_ptr];
...
int circular_buffer_putc(circular_buffer_t *circular_buffer_p,
                         int *putc_char_p) {
  int r;
  int write_count;
  ...
  if (circular_buffer_p->calculated_buffer_max_writeable_chunk >= 1) {
    ...
    if (circular_buffer_p->calculated_buffer_wrapped_write_start_ptr >=
        circular_buffer_p->circular_buffer_allocated_size) {
...
int circular_buffer_block_transfer(circular_buffer_t *circular_buffer_1_p,
  ...
  max_buffer_transfer =
      circular_buffer_1_p
          ->calculated_buffer_max_readable_chunk; /* most we can read from
                                                     buffer_01 */
  if (circular_buffer_2_p->calculated_buffer_max_writeable_chunk <=
      max_buffer_transfer) { /* most we can transfer to buffer_02 */
    max_buffer_transfer =
        circular_buffer_2_p
            ->calculated_buffer_max_writeable_chunk; /* most we can transfer
                                                        from buffer_01 to
                                                        buffer_02 */
  }
...
// @@@
    ...
    printf("\tyou can transfer %d bytes from %s[%d] to %s[%d], \"",
           max_buffer_transfer, circular_buffer_1_p->buffer_name,
    ...
    for (i = 0; i < max_buffer_transfer; ++i) {
      ...
      putc(circular_buffer_1_p->circular_buffer_data_p
    ...
    memcpy(
        &circular_buffer_2_p->circular_buffer_data_p
             [circular_buffer_2_p->calculated_buffer_wrapped_write_start_ptr],
        &circular_buffer_1_p->circular_buffer_data_p
             [circular_buffer_1_p->unwrapped_read_start_ptr],
        max_buffer_transfer);
    ...
    CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(circular_buffer_1_p);
    if (circular_buffer_1_p->unwrapped_read_start_ptr >=
        circular_buffer_1_p->circular_buffer_allocated_size) {
      // WRAP@@@@@@@
      CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(circular_buffer_1_p);
      circular_buffer_1_p->unwrapped_read_start_ptr %=
          circular_buffer_1_p->circular_buffer_allocated_size;
...
#ifdef CIRCULAR_BUFFER_WRAP_DEBUG
      printf("FIX_2: %s-%d: "
             "circular_buffer_1_p->unwrapped_read_start_ptr==%d" NEWLINE,
             __FILE__, __LINE__, circular_buffer_1_p->unwrapped_read_start_ptr);
#endif
    }
...
int circular_buffer_multi_block_transfer(circular_buffer_t *circular_buffer_1_p,
                                         circular_buffer_t *circular_buffer_2_p,
                                         int max_multi_buffer_transfer) {
  int i;
  int r;
  ...
  if (circular_buffer_1_p->calculated_buffer_max_writeable_chunk <=
      max_buffer_transfer) { /* most we can transfer to buffer_02 */
    max_buffer_transfer =
        circular_buffer_1_p
            ->calculated_buffer_max_readable_chunk; /* most we can read from
  ...
  if (max_buffer_transfer != max_multi_buffer_transfer) {
    printf("ERROR_3: %s-%d: max_multi_buffer_transfer==%d, "
           "max_buffer_transfer==%d" NEWLINE,
           __FILE__, __LINE__, max_multi_buffer_transfer, max_buffer_transfer);
  }
...
int circular_buffer_multi_block_transfer_complete(
    circular_buffer_t *circular_buffer_1_p, int max_multi_buffer_transfer) {
  int max_buffer_transfer;
  ...
  if (max_buffer_transfer >= 1) {
    ...
    for (i = 0; i < max_buffer_transfer; ++i) {
      ...
      current_char = circular_buffer_1_p->circular_buffer_data_p
                         [circular_buffer_1_p->unwrapped_read_start_ptr + i];

      printable_buff[0] = '\0';

      r = get_ascii_control_code_info(current_char, &printable_char_abrev,
                                      NULL);
      ...
      if (r == 0) {
        ...
        sprintf(printable_buff, "<%s>", printable_char_abrev);
        ...
        if (current_char >= 0x20 && current_char <= 0x7e) {
          sprintf(printable_buff, "\"%c\"", current_char);
        } else {
      ...
      printf("DUMP: %s-%d: index==%d/size==%d, ptr==%d, char==0x%02x-%s",
             __FILE__, __LINE__, i, max_buffer_transfer,
             circular_buffer_1_p->unwrapped_read_start_ptr + i, current_char,
             printable_buff);
...
int circular_buffer_math_calculations(
    int circular_buffer_allocated_size,
    int unwrapped_read_start_ptr, /* first used location */
    int circular_buffer_used_bytes, int *max_readable_chunk_p,
...
// WRAP@@@@@@@
  ...
  if (unwrapped_read_start_ptr >= circular_buffer_allocated_size) {
    printf("ERROR_2: %s-%d: unwrapped_read_start_ptr==%d, "
           "circular_buffer_allocated_size==%d" NEWLINE,
           __FILE__, __LINE__, unwrapped_read_start_ptr,
  ...
  if (unwrapped_read_start_ptr < 0) {
    // CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(create_circular_buffer_p);
    printf("ERROR_2: %s-%d: unwrapped_read_start_ptr<0==%d, "
           "circular_buffer_allocated_size==%d" NEWLINE,
           __FILE__, __LINE__, unwrapped_read_start_ptr,
           circular_buffer_allocated_size);
    //@@@THIS_IS_NOT_THE_CORRECT_PLACE_TO_FIX_WRAP:
  ...
  if (circular_buffer_used_bytes == 0) {
    ...
    unwrapped_read_end_ptr =
        (unwrapped_read_start_ptr); /* last used read location, no data */

    // CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(create_circular_buffer_p);
    unwrapped_write_start_ptr =
        (unwrapped_read_start_ptr +
         circular_buffer_used_bytes); /* next free write location */
    // CONNSAT_BUFFER_TRACE_UNWRAPPED_READ_START_PTR(create_circular_buffer_p);
    ...
    unwrapped_read_end_ptr =
        (unwrapped_read_start_ptr + circular_buffer_used_bytes -
         1); /* last used read location */
  ...
  wrapped_write_start_ptr =
      unwrapped_write_start_ptr % circular_buffer_allocated_size;
...
#ifdef CIRCULAR_BUFFER_ALGORITHM_DEBUG
  printf("alocated_bytes==%d, used_bytes==%d, read_start_ptr==%d, "
         "max_read_bytes==%d, write_start_ptr==%d(%d), "
         "max_write_bytes==%d" NEWLINE,
         circular_buffer_allocated_size, circular_buffer_used_bytes,

         unwrapped_read_start_ptr, max_readable_chunk_data,

         wrapped_write_start_ptr, unwrapped_write_start_ptr,
         max_writeable_chunk_data);
#endif
  ...
  if (unwrapped_read_start_ptr <= wrapped_write_start_ptr) {
    ...
    if (circular_buffer_used_bytes == 0) {
      ...
      if ((wrapped_write_start_ptr + max_writeable_chunk_data) >
          circular_buffer_allocated_size) {
        max_writeable_chunk =
            circular_buffer_allocated_size - wrapped_write_start_ptr;
      } else {
    ...
    if (circular_buffer_used_bytes == 0) {
      ...
      chunk_range_readable_end =
          unwrapped_read_start_ptr + max_readable_chunk - 1;
    }
...
#ifdef CIRCULAR_BUFFER_ALGORITHM_DEBUG
  printf("\tmax_readable_chunk==%d, [%d::%d]\t\t%s" NEWLINE, max_readable_chunk,
         chunk_range_readable_start, chunk_range_readable_end,
         marker_string_read_p);

  printf("\tmax_writeable_chunk==%d, [%d::%d]\t\t%s" NEWLINE,
         max_writeable_chunk, chunk_range_writeable_start,
         chunk_range_writeable_end, marker_string_write_p);
#endif
