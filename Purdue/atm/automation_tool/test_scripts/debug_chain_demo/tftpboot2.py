...
def wait_for_prompt(process, expected_prompt, count=None, meta_char=None):
    ...
    while True:
        ...
        try:
            ...
            print(
                f"With attempt {attempts}. Can't match the prompt and timeout reached...."
            )
            process.send("\n\r")
            attempts += 1
...
conn = pexpect.spawn(
    "/home/nixos/automation_tool/connsat_client/connsat_client -p 50002",
    timeout=10,
    encoding="utf-8",
)
time.sleep(1)
...
prompts = [r"loader> ", r"# "]
cmds = [
    "set ip 172.25.27.31\r",
    "set netmask 255.255.255.0\r",
    "set gw 172.25.27.225\r",
    "boot tftp://172.25.27.208/ucs-6400-k9-system.10.4.2.I43.6a.45.F.gbin\r",
]
for each in cmds:
    prompt = prompts[0]
