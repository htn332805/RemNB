...
// static char *short_options = ":TtKkA::aDdL:lR:S:sNni:p:Vh?";
static char *short_options = ":i:p:vVh?";
...
printf("Option 0x%02x: optind==%d, opterr==%d, optopt==%d, optarg==0x%p%s",
	...
	newline_string_p);
		...
		// static char *short_options = ":TtKkA:aDdL:lR:S:sNni:p:V?";
		switch (c) {
			case 0:
#if 0 /* debug stuff */
			...
			case 'T':
				...
				connsat_options_p->trace_buffer_transfer_flag = 1;
				break;
			case 't':
				printf("option t%s", newline_string_p);
				connsat_options_p->trace_buffer_transfer_flag = 0;
			...
			case 'K':
				...
				connsat_options_p->trace_buffer_calculations_flag = 1;
				break;
			case 'k':
				printf("option k%s", newline_string_p);
				connsat_options_p->trace_buffer_calculations_flag = 0;
				break;
			case 'A':
				connsat_options_p->alive_feature_enable_flag = 1;

				if (optarg != NULL) { /* optional argument */

					connsat_options_p->alive_feature_command_string_p = optarg;
			...
			case 'D':
				...
				connsat_options_p->debug_ascii_characters_flag = 1;
				break;
			case 'd':
				printf("option d%s", newline_string_p);
				connsat_options_p->debug_ascii_characters_flag = 0;
			...
			case 'L':
				...
				connsat_options_p->session_buffer_enable_flag = 1;
				connsat_options_p->session_log_path_p = optarg;
				break;
			case 'l':
			...
			case 'R':
				...
				connsat_options_p->capture_buffer_enable_flag = 1;
				connsat_options_p->capture_log_path_p = optarg;
				break;
			case 'S':
				printf("option S with value `%s'%s", optarg, newline_string_p);
				connsat_options_p->spawnshell_enable_flag = 1;
				connsat_options_p->spawnpty_shell_command_p = optarg;
				break;
			case 's':
				printf("option s%s", newline_string_p);
				connsat_options_p->spawnshell_enable_flag = 0;
				break;
			case 'N':
			...
			case 'n':
				printf("option n%s", newline_string_p);
				connsat_options_p->network_server_enable_flag = 0;
				break;
#endif
			case 'i':
#if 0 /* debug stuff */
...
#endif
				connsat_options_p->network_ip_addr_string_p = optarg;
				break;
			case 'p':
#if 0 /* debug stuff */
...
#endif
				// connsat_options_p->xxxxxxxx = 1;

				connsat_options_p->display_version = 1;
			...
			case ':':
				...
				*/
				...
				connsat_options_p->connsat_help_flag = 1;
	...
	if (connsat_options_p->display_version != 0) {
	...
	if (connsat_options_p->connsat_help_flag != 0) {
