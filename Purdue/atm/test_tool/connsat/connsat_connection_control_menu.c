...
#if 0
#define CONNSAT_CONTROL_MENU_TRACE printf("CM_TRACE: %s-%d%s", __FILE__, __LINE__, newline_string_p)
#else
#define CONNSAT_CONTROL_MENU_TRACE
#endif

int connsat_connection_control_menu_buffer_distribution_worker(
...
int
connsat_connection_control_menu (
	struct connsat_connection_s **connsat_connection_control_menu_pp,
	int menu_mode_flag,
	int raw_mode_flag
	)
{
	...
	r = connsat_connection_new(
		"Control-Menu-Connnection-Channel",
		CONNSAT_CONNECTION_TYPE_CONTROL_MENU, 	// CONNSAT_CONNECTION_TYPE_NONE,
		102400,					/* input buffer size */
		204800,					/* output buffer size */
		raw_mode_flag,

		connsat_connection_control_menu_input_open,	/* connection_input_open_p */
		connsat_connection_control_menu_input_close,	/* connection_input_close_p */

		connsat_connection_control_menu_output_open,	/* connection_output_open_p */
		connsat_connection_control_menu_output_close,	/* connection_output_close_p */

		connsat_connection_control_menu_input_read,	/* connection_input_read_p */
		connsat_connection_control_menu_output_write,	/* connection_output_write_p */

		connsat_connection_control_menu_buffer_distribution, /* buffer_distribution_p */

		connsat_connection_control_menu_pp		/* connsat_connection_new_p */
		);
	...
	printf("\tCREATED: connsat_connection_control_menu_p: %p \"%s\"%s",
		*connsat_connection_control_menu_pp,
		(*connsat_connection_control_menu_pp)->name,
		newline_string_p);
#endif

#if 1
	r = connsat_connection_control_menu_input_open(
		*connsat_connection_control_menu_pp,
		menu_mode_flag,
		raw_mode_flag);

	r = connsat_connection_control_menu_output_open(
		*connsat_connection_control_menu_pp,
		menu_mode_flag,
		raw_mode_flag);
#endif
...
int
connsat_connection_control_menu_input_open (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
	...
	connsat_connection_control_menu_p->connection_input_fd = -1;	/* NOT_USED: stdin-fd */

	connsat_connection_control_menu_p->connection_input_state = CONNSAT_CONNECTION_STATE_OPEN_ACTIVE;

	/* don't add the input_fd stream to the POLLIN list */
	connsat_connection_control_menu_p->connection_input_fd_poll_flag = 0;

	/* enable the input buffer to receive characters */
	connsat_connection_control_menu_p->connection_input_buffer_enable_flag = 1;

	/* enables receiving buffer distributions from other input buffers */
	connsat_connection_control_menu_p->connection_input_buffer_distribution_flag = 1;
...
int
connsat_connection_control_menu_input_close (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
	...
	/* buffer only connection */
	connsat_connection_control_menu_p->connection_input_state = CONNSAT_CONNECTION_STATE_CLOSED;

	/* don't add the input_fd stream to the POLLIN list */
	connsat_connection_control_menu_p->connection_input_fd_poll_flag = 0;

	/* disable the input buffer to receive characters */
	connsat_connection_control_menu_p->connection_input_buffer_enable_flag = 0;

	/* disable receiving buffer distributions from other input buffers */
	connsat_connection_control_menu_p->connection_input_buffer_distribution_flag = 0;
...
int
connsat_connection_control_menu_output_open (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
	...
	/* buffer only connection */
	connsat_connection_control_menu_p->connection_output_fd = -1; /* NOT_USED: stdout-fd */

	connsat_connection_control_menu_p->connection_output_state = CONNSAT_CONNECTION_STATE_OPEN_ACTIVE;

	/* don't add the output_fd stream to the POLLOUT list */
	connsat_connection_control_menu_p->connection_output_fd_poll_flag = 0;

	/* enable the output buffer to receive characters */
	connsat_connection_control_menu_p->connection_output_buffer_enable_flag = 1;

	/* enables receiving buffer distributions from other input buffers */
	connsat_connection_control_menu_p->connection_output_buffer_distribution_flag = 1;

	/* SELF_LOOPBACK */
	connsat_connection_control_menu_p->connection_distribution_self_loopback_enable_flag = 0;
...
int
connsat_connection_control_menu_output_close (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
	...
	/* buffer only connection */
	connsat_connection_control_menu_p->connection_output_state = CONNSAT_CONNECTION_STATE_CLOSED;

	/* don't add the output_fd stream to the POLLOUT list */
	connsat_connection_control_menu_p->connection_output_fd_poll_flag = 0;

	/* disable the output buffer to receive characters */
	connsat_connection_control_menu_p->connection_output_buffer_enable_flag = 0;

	/* disable receiving buffer distributions from other input buffers */
	connsat_connection_control_menu_p->connection_output_buffer_distribution_flag = 0;
...
int
connsat_connection_control_menu_input_read (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
	...
	circular_buffer_calculations(connsat_connection_control_menu_p->connection_input_buffer_p);

	small_input_buffer_max_writeable_chunk = connsat_connection_control_menu_p->connection_input_buffer_p->max_writeable_chunk;
...
//@@@@@@@ DEBUG
	...
	r = connsat_connection_control_menu_buffer_distribution_worker(
...
/* write control_menu INPUT from large output distribution buffer */
//NO-POLLOUT
int
connsat_connection_control_menu_output_write (
	struct connsat_connection_s *connsat_connection_control_menu_p,
	int menu_mode_flag,
	int raw_mode_flag)
{
...
/* process large output buffer chars into control_menu input */
...
int
connsat_connection_control_menu_buffer_distribution (
	struct connsat_connection_s *connsat_connection_control_menu_p,
		/* FROM CONNECTION */
	...
	int raw_mode_flag)
{
	...
	// CONNSAT_TRACE;
	CONNSAT_CONTROL_MENU_TRACE;

	if (connsat_connection_control_menu_p->connection_type != CONNSAT_CONNECTION_TYPE_CONTROL_MENU) {

		printf("Error: %s-%d, connection is not CONNSAT_CONNECTION_TYPE_CONTROL_MENU<%s", __FILE__, __LINE__,
			newline_string_p);
...
/* SET */		&buffer_distribution_max_writeable_chunk, /* calculated output, destination_write_buffer */
		...
		if (r != 0) {
			...
			printf("Error: %s-%d, buffer transfer calculation error for CONNECTION_TYPE_CONTROL_MENU<%s",
				__FILE__, __LINE__,
				newline_string_p);
...
printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ %s-%d%s", __FILE__, __LINE__, newline_string_p);
		...
		if (distribution_mode_flag & BUFFER_DISTRIBUTION_MODE_FLAG_CALCULATION_DISPLAY_MODE) {
			...
			printf("\t\t\t%s: %s-%d, CONNECTION_TYPE_CONTROL_MENU-ALL_BUFFERS >>%d:%d<<%s",
...
/* SAVE */		*calculation_mode_max_readable_chunk_p = buffer_distribution_max_readable_chunk;
			...
			printf("\t\t\t\t\t\tSAVED_DISTRIBUTION_control_menu_CALCULATION_MODE_MAX_READABLE_CHUNK__DISTRIBUTION_INPUT: %s-%d, max_readable_chunk==%d%s",
				__FILE__, __LINE__,
				buffer_distribution_max_readable_chunk,
				newline_string_p);
#endif
...
#ifdef MASTER_DEBUG_FLAG_01_SAVED
			printf("\t\t\t\t\t\tSAVED_DISTRIBUTION_control_menu_CALCULATION_MODE_MAX_WRITEABLE_CHUNK__DISTRIBUTION_OUTPUT: %s-%d, max_writable_chunk==%d%s",
				__FILE__, __LINE__,
				buffer_distribution_max_writeable_chunk,
				newline_string_p);
#endif
...
/* USE */				&buffer_distribution_max_writeable_chunk, /* calculated input, destination_write_buffer */
				...
				if (r != 0) {
					...
					printf("Error: %s-%d, buffer transfer copy error for CONNECTION_TYPE_CONTROL_MENU<%s",
						__FILE__, __LINE__,
						newline_string_p);
...
printf("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ %s-%d%s", __FILE__, __LINE__, newline_string_p);
		...
		if (distribution_mode_flag & BUFFER_DISTRIBUTION_MODE_FLAG_TRANSFER_MODE) {
			printf("Error: %s-%d, can't perform buffer transfer without perform calculations first for CONNECTION_TYPE_CONTROL_MENU<%s",
				__FILE__, __LINE__,
				newline_string_p);
...
int
connsat_connection_control_menu_buffer_distribution_worker (
	struct connsat_connection_s *connsat_connection_control_menu_p,
		/* FROM CONNECTION */
		/* is a CONNSAT_CONNECTION_TYPE_TERMINAL */
	int distribution_mode_flag,
...
#if 0
printf("\t\t\t%s: %s-%d, CONNECTION_TYPE_CONTROL_MENU-smallbuffer[%d/%d] >>%d<< %s %d%s",
	"DEBUG",
	__FILE__, __LINE__,
	connsat_connection_control_menu_p->connection_input_buffer_p->circular_buffer_used_bytes,
	connsat_connection_control_menu_p->connection_input_buffer_p->circular_buffer_allocated_size,
	max_readable_chunk,
	connsat_connection_control_menu_p->connection_input_buffer_p->buffer_name,
	connsat_connection_control_menu_p->connection_input_buffer_p->unwrapped_read_start_ptr,
	newline_string_p);
#endif
					...
					/* source_read_buffer */
					max_readable_chunk = connsat_connection_control_menu_p->connection_input_buffer_p->calculated_buffer_max_readable_chunk;
...
CONNSAT_TRACE_DUMP;
							...
							printf("%s\t\t\t%s: %s-%d, CONNECTION_TYPE_CONTROL_MENU-smallbuffer[%d/%d] >>%d<<%s",
								newline_string_p,
								from_string_p,
								__FILE__, __LINE__,
								connsat_connection_control_menu_p->connection_input_buffer_p->circular_buffer_used_bytes,
					...
					if (distribution_mode_flag & BUFFER_DISTRIBUTION_MODE_FLAG_TRANSFER_MODE) {
						...
						printf("%s-%d, MULTI_TRANSFER: CONTROL_MENU->TERMINAL max_buffer_transfer==%d%s",
							__FILE__, __LINE__,
							max_buffer_transfer,
							newline_string_p);
#endif
...
CONNSAT_TRACE_DUMP;
							...
							r = circular_buffer_multi_block_transfer(
								connsat_connection_control_menu_p->connection_input_buffer_p,
...
#ifdef MASTER_DEBUG_FLAG_01_MULTI_TRANSFER
						printf("%s-%d, MULTI_TRANSFER: CONTROL_MENU->SIDEBAND_CHANNEL  max_buffer_transfer==%d%s",
							__FILE__, __LINE__,
							max_buffer_transfer,
							newline_string_p);
#endif
...
CONNSAT_TRACE_DUMP;
				...
				if (distribution_mode_flag & BUFFER_DISTRIBUTION_MODE_FLAG_TRANSFER_DISPLAY_MODE) {
					...
					printf("%s\t\t\t\t%s: %s-%d, CONNECTION_TYPE_CONTROL_MENU-smallbuffer[%d/%d] <<<%d>>> TRANSFER-COMPLETE%s",
						newline_string_p,
						from_string_p,
						__FILE__, __LINE__,
						connsat_connection_control_menu_p->connection_input_buffer_p->circular_buffer_used_bytes,
...
#if 0	/* no control menu buffer distribution in normal menu mode */
